...
outputPath="Arc_collision_bdls_NoArc_7e-5"
...
OP=fuseListM([OP[0],OP[1],OP[2],OP[3],OP[4],OP[5]])
...
Model.surfaceProcesses = GEO.surfaceProcesses.Badlands(airIndex=[Air.index],sedimentIndex=sediment.index, XML="./ressources/badlands3.xml", resolution=4. * u.kilometer,restartFolder="outbdls", checkpoint_interval=0.2 * u.megayears,restartStep=197)
...
if restart==False:
    ...
    run_for_adaptative(Model,duration=(20.*MaUnit)*u.year,checkpoint_interval=(0.2*MaUnit)*u.year,dt=5000.*u.years,restartDir=outputPath,restartStep=196)
...
#            #store.step += 1
#
#        #This removes plastic strain everywhere in each timestep (except the trench) until the subduction zone reach the equilibrium and the velocity field stabilizes
#        plasticStrain = Model.swarm_variables['plasticStrain']
#        #weakMat = weakShape.evaluate(Model.swarm)
#        weakMat = weakShape2.evaluate(Model.swarm)
#        Model.swarm_variables['plasticStrain'].data[:] = np.where(weakMat == False,
#                                                                      0., plasticStrain.data )
#
#         #This removes plastic damage from model walls
#        fact1=InfplasticFn.evaluate(Model.swarm)
#        Model.plasticStrain.data[:] =Model.plasticStrain.data[:] * fact1
#
#
#
#    Model.post_solve_functions["B"] = post_hook
#    ##First Run that removes plastic strain at the end of every time step everywhere except in the trench.
#    run_for_adaptative(Model,duration=(0.2*MaUnit)*u.year,checkpoint_interval=(0.2*MaUnit)*u.year,dt=1500*u.years)
#    #After 400 Kyr the model reaches the equilibrium and we set the Equilibrium variable as True.
#    Equilibrium=True
#    #We let the subduction zone evolve before and increase the time Step
#    run_for_adaptative(Model,duration=(6.*MaUnit)*u.year,checkpoint_interval=(0.2*MaUnit)*u.year,dt=7000*u.years,restartDir=outputPath)
#
## When re-starting the model, the Equilibrium variable has to be set to True. This allows the code to run the function bellow, where the re-start step
## can be specified and where plastic strain is just healed in the model walls.
#if Equilibrium==True:
#
#    ## Second run that enables plasticity in all the model domain
#    def post_hook():
#        vrms = Model.stokes_SLE.velocity_rms()
#        step = Model.step
#        time = Model.time.m_as(u.year)
...
#    Model.post_solve_functions["B"] = post_hook
#
##    run_for_adaptative(Model,duration=(39.*MaUnit)*u.year,checkpoint_interval=(0.05*MaUnit)*u.year,dt=7000*u.years,restartDir=outputPath,restartStep=72)
#
#    run_for_adaptative(Model,duration=(39.*MaUnit)*u.year,checkpoint_interval=(0.2*MaUnit)*u.year,dt=8000*u.years,restartDir=outputPath)
#################
